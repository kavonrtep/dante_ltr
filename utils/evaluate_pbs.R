...
# testing
#f <- "/mnt/ceph/454_data/dante/Intact_LTR_dataset/assembly_o_sativa
# /Oryza_sativa_msu7_dante_ltr_bugfix32_trna_hemi_27.gff3"

suppressPackageStartupMessages(library(rtracklayer))
gff <- import(f, format = "gff3")
eval <- as.numeric(gff$PBS_evalue)

gff_te <- gff[gff$type == "transposable_element" &
                !grepl("partial", gff$ID) &
                !is.na(eval),]
...
for (n in names(gff_te_p)) {
  ...
  eval <- as.numeric(gff_te_p[[n]]$PBS_evalue)
  AAp <- sapply(strsplit(sapply(strsplit(pbs, "__"), "[[", 2), "-"), "[[", 1)
  AA_levels <- names(sort(table(AAp)))
  if (all(eval >= 0.1)) {
    AAp01 <- character(0)
  }else {
    AAp01 <- sapply(strsplit(sapply(strsplit(pbs[eval < 0.1], "__"), "[[", 2), "-"),
                    "[[", 1)
  }
  if (all(eval >= 0.5)) {
    AAp05 <- character(0)
  }else {
    AAp05 <- sapply(strsplit(sapply(strsplit(pbs[eval < 0.5], "__"), "[[", 2), "-"),
                    "[[", 1)
  }
  par(mfrow = c(3, 1), mar = c(4, 20, 2, 2))
  barplot(sort(table(gff_te_p[[n]]$trna_id), decreasing = TRUE), main = n, horiz =
    TRUE, las = 1)
  ...
  legend("bottomright", legend = c("all", "evalue < 0.5", "evalue < 0.1"), fill = c
  ("gray", "orange", "red"))
  hist(-log10(as.numeric(gff_te_p[[n]]$PBS_evalue)), xlab = "-log10 evlaue", main = n,
       breaks = 100)
}
par(mfrow = c(3, 1))
hist(-log10(as.numeric(gff_te$PBS_evalue)), xlab = "-log10 evlaue", main = "all",
     breaks = 20)
