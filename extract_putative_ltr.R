...
TE <- mclapply(seq_along(gr), function(x)get_TE(s_left[x],
                                                ...
                                                grL[x],
                                                grR[x]),
               mc.set.seed = TRUE, mc.cores = 10, mc.preschedule = FALSE
)
# TODO - change mc.cores back to opt$cpu
...
add_pbs <- function(te, s, trna_db) {
  ...
  system(cmd)
  pbs_exact_gr <- NULL
  out_raw <- read.table(tmp_out, as.is = TRUE, sep = "\t", col.names = strsplit("qaccver saccver pident length mismatch gapopen qstart qend sstart send evalue bitscore qseq sseq", split = ' ')[[1]])
  if (nrow(out_raw) > 0) {
